...
3. `ALCHEMY_API_KEY` — the CLI builds the Alchemy URL for known chains
...
| `--rpc <url>` | — | RPC URL (else `PAMPALO_RPC` / `ALCHEMY_API_KEY`) |
| `-p, --passphrase <s>` | — | Keystore passphrase (else prompt / env) |
