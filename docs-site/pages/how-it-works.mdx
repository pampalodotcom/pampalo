...
The note's creator generates the **secret** and ECIES-encrypts it to the recipient's **Envelope key**. Only the ciphertext rides on-chain with the leaf, so the server and relayer never see the plaintext. The recipient's wallet **scans** events, decrypts with its Envelope private key, and files the note in local storage.
